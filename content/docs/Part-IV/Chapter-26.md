...
<p style="text-align: justify;">
The time complexity of the Hopcroft-Karp Algorithm is $O(E \sqrt{V})$, where $E$ is the number of edges and $V$ is the number of vertices. This efficiency comes from the fact that each phase (BFS and DFS) is performed in $O(E)$ time, and the algorithm typically performs $O(\sqrt{V})$ iterations.
</p>
